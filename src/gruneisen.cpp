...
void singleCrystalBuilder(boost::mpi::communicator world) {
    ...
    if (filename.compare("AUTO") == 0) {
        ...
        buffer << compound << "_" << gridDensityA << "_" << gridDensityB << "_"
               << gridDensityC << "_" << std::to_string(Temperature) << ".gruneissen";
        filename = buffer.str();
    }

    auto out_target_file = output_dir / boost::filesystem::path(filename);

    // IF OVERWRITE IS TURNED OFF, MAKE SURE TARGETED HDF5 FILE DOESN'T EXIST
    // YET.

    if (!overwrite) {
        if (boost::filesystem::exists(out_target_file)) {
            std::cout << std::endl;
            std::cout
    ...
    std::cout << "***********************************" << std::endl;
    std::cout << "This is ALMA/gruneisen version " << ALMA_VERSION_MAJOR << "."
    ...
    auto gruneisen = calc_mode_gruneisen(*poscar, *grid, *thirdorder);
    std::ofstream gruneisen_file;
    gruneisen_file.open(out_target_file.c_str());
    gruneisen_file << "# Total gruneissen : " << calc_total_gruneisen(*grid, gruneisen, Temperature) << std::endl;
    gruneisen_file << "# Mode decomposed non-T-weighted:\n";
    gruneisen_file << "# iq, im, omega, qx, qy, qz, gruneisen\n";
    const auto nmodes = grid->get_spectrum_at_q(0).omega.size();
    for (auto iq = 0; iq < grid->nqpoints; iq++) {
        auto spectrum = grid->get_spectrum_at_q(iq);
        auto qvec = grid->get_q(iq);
        for (auto im = 0; im < nmodes; im++) {
            gruneisen_file << iq << ',' << im << ',' <<  spectrum.omega(im) << ',' <<
                        qvec(0) << ',' << qvec(1) << ',' << qvec(2) << ',' << gruneisen(im,iq) << std::endl;
        }
    } 
    gruneisen_file.close();

    std::cout << "Total gruneissen : " << calc_total_gruneisen(*grid, gruneisen, Temperature) << std::endl;
...
int alloyBuilder() {
    ...
    target_file_builder << "_" << gridDensityA << "_" << gridDensityB << "_"
                        << gridDensityC << "_" << std::to_string(Temperature) << ".gruneisen";
    ...
    auto out_target_file = alloy_dir / boost::filesystem::path(target_filename);
    ...
    /// Compute Gruneisen parameter
    auto gruneisen = calc_mode_gruneisen(*vc_poscar, *grid, *vc_thirdorder);
    std::ofstream gruneisen_file;
    gruneisen_file.open(out_target_file.c_str());
    gruneisen_file << "# Total gruneissen : " << calc_total_gruneisen(*grid, gruneisen, Temperature) << std::endl;
    gruneisen_file << "# Mode decomposed non-T-weighted:\n";
...
int main(int argc, char** argv) {
    ...
    if (world.size() > 1) {
        throw std::runtime_error("gruneissen can only be run in serial");
    }
    ...
    if (argc < 2) {
        std::cout << "usage: gruneisen <inputfile.xml> T"
                  << std::endl;
        return 1;
    }
    ...
    if (Temperature < 0.0) throw alma::value_error("Temperature cannot be negative");
